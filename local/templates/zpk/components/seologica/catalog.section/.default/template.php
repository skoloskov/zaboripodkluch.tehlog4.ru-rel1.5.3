...
foreach ($arResult['ITEMS'] as $item)
{
	...
	$objPhoto = CFile::ResizeImageGet($item["PROPERTIES"]['PHOTO']['VALUE'][0], array('width' => 800, 'height' => 600), BX_RESIZE_IMAGE_EXACT, true);
	?>
	<li class="territory__item">
		<a class="territory__link " href="<?=$item['DETAIL_PAGE_URL']?>" style="background-image:url(<?=$objPhoto['src']?>)">
			<!-- cards 1 -->
			<div class="territory__item-content">
				<div class="territory__item-header">
					<div class="territory__item-date"><?=date('d.m.y',strtotime($item['DATE_CREATE']))?></div>
					...
					<span class="range__text">рейтинг: <?=(int)$item['SHOW_COUNTER']?></span>
				</div>
				...
				<div class="territory__item-desc">
					...
					<div class="territory__desc-content">
						...
						<div class="territory__el_number"><?=$item['PROPERTIES']['PRICE']['VALUE']?> р</div>
					</div>
				</div>
			</div>
		</a>
