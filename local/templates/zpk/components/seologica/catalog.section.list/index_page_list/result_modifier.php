...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	//if ($boolPicture || $boolDescr)
	{
		...
		$rsSections = CIBlockSection::GetList(array(), array('IBLOCK_ID'=>$arParams['IBLOCK_ID'],'ID' => array_keys($arMap)), false, $arSelect);
		while ($arSection = $rsSections->GetNext())
		{
			...
			foreach($arSection as $ukey=>$uvalue){
				if (strpos($ukey,'UF_')!==false){
					$arResult['SECTIONS'][$key]['IPROPERTY_VALUES'][$ukey] = $uvalue;
				}
			}
			if ($boolPicture)
			{
