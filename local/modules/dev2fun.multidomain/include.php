...
class Base {
	...
	public static function InitDomains() {
		...
		if(!$domain = $oSubDomain->check()){
			//echo '<pre>'.print_r($_SERVER['HTTPS'], true).'</pre>';die();
			header('Location: http'.($_SERVER['HTTPS']=='on' ? 's' : '').'://'.SITE_SERVER_NAME.'/404.php'); die();
			return true;
		}
...
//			}
	...
	public static function InitSeoDomains() {
		...
		if($USER->IsAdmin() && false) { // Отключил кнопку управления данными компонента из публичной части, чтобы не смущала
